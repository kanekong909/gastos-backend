...
// ── SCANNER DE RECIBO (Google Gemini) ────────────────
const GEMINI_API_KEY = 'TU_API_KEY_AQUI'; // ← pega tu key de aistudio.google.com
const GEMINI_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${GEMINI_API_KEY}`;

document.getElementById('btn-scan-recibo').addEventListener('click', () => {
  document.getElementById('scan-recibo-input').click();
});

document.getElementById('scan-recibo-input').addEventListener('change', async function () {
  const file = this.files[0];
  if (!file) return;

  const statusEl = document.getElementById('scan-recibo-status');
  const btnScan  = document.getElementById('btn-scan-recibo');

  statusEl.textContent = '🔍 Analizando recibo…';
  statusEl.classList.remove('hidden');
  statusEl.style.color = '';
  btnScan.disabled = true;
  btnScan.textContent = 'Analizando…';
      ...
      body: JSON.stringify({
        contents: [{
          parts: [
            ...
            {
              text: `Analiza este recibo o factura. Responde ÚNICAMENTE con un JSON válido, sin texto adicional, sin backticks, con este formato:
{"monto": <número entero en pesos colombianos sin puntos ni comas, o null>, "fecha": "<YYYY-MM-DD o null>"}
Reglas: si el monto tiene punto como separador de miles (ej 15.500) trátalo como 15500. Si hay varios totales, usa el total final. Si la fecha no tiene año, asume el año actual.`
            }
          ]
        }],
    ...
    if (result.monto && result.monto > 0) {
      const el = document.getElementById('f-monto');
      el.value = Number(result.monto).toLocaleString('es-CO');
    ...
    if (rellenos.length > 0) {
      statusEl.textContent = `✓ Se detectó: ${rellenos.join(' y ')}`;
      statusEl.style.color = 'var(--green)';
    } else {
      statusEl.textContent = '⚠ No se encontró monto ni fecha';
      statusEl.style.color = 'var(--accent)';
    }

  } catch (e) {
    console.error('Error escaneando recibo:', e);
    statusEl.textContent = '✗ No se pudo leer el recibo';
    statusEl.style.color = 'var(--red)';
  } finally {
    btnScan.disabled = false;
    btnScan.textContent = '📷 Escanear recibo';
    this.value = '';
    setTimeout(() => {
